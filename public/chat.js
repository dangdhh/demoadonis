...
function startChat() {
  ...
  ws.on("open", () => {
    ...
    ws.getSubscription("chat").emit("register", { username: user });
    subscribeToChannel();
  });
...
function subscribeToChannel() {
  ...
  chat.on("message", message => {
    ...
    if (window.username == message.username) {
      const html =
        `<div class="message-group me" username="` +
        message.username +
        `">
      ...
      const html =
        `<div class="message-group" username="` +
        message.username +
        `">
                <img src="images/no-avatar.png" class="message-avatar" />
                <div class="message-body">
                    <h4 class="message-username">` +
        message.username +
        `</h4>
                    <div id="messages">
                        <div class="including">
    ...
    if (sameLastMessage) {
      $("#listMessages > div.message-group:last")
        .find(".message:last")
        .removeClass("bottom");
      $("#listMessages > div.message-group:last")
        .find(".message:last")
